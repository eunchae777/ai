-- [ III ] JOIN : 2개 이상 테이블을 연결하여 데이터를 검색하는 방법
...
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO, DNAME, LOC
    ...
    -- ex. 급여가 2000가 이상인 직원의 이름, 직책, 급여, 부서명, 근무지
    SELECT ENAME, JOB, SAL, DNAME, LOC FROM EMP E, DEPT D; -- E 14개 X  D4개
    ...
    -- ex. JOB이 'SALESMEN'이거나 'MANAGER'인 사원의 이름, 급여, 상여, 연봉, 부서명
        -- 연봉이 큰 순으로 정렬. 단, 연봉 = (SAL+COMM)*12
    SELECT ENAME, SAL, COMM, (SAL + NVL(COMM,0) )*12  ANNUALSAL, DNAME  
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND JOB IN ('SALEMAN', 'MANAGER')
...
SELECT E.ENAME, E.SAL, E.DEPTNO, LOC
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO
...
SELECT ENAME, COMM, DNAME, LOC 
  FROM EMP E, DEPT D
  WHERE E.DEPTNO=D.DEPTNO AND COMM>0; -- COMM!=0 또는 (COMM!=0 AND COMM IS NOT NULL)
