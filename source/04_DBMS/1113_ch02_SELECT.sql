...
SELECT EMPNO  "사 번", ENAME  "이 름", SAL  "급여", JOB  
    FROM EMP; -- 열이름에 별칭을 두는 경우 EX. 열이름 "별칭", 열이름 별칭
...
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL<>3000;
    ...
    -- es3. 부서번호(DEPTNO)가 10번인 사원의 모든 필드
    SELECT * FROM EMP WHERE DEPTNO=10;
    -- SQL문은 대소문자 구별없음. 데이터는 대소문자 구별
    -- es4. 이름이(ENAME)이 SCOTT인 직원의 모든 데이터(필드)
...
SELECT EMPNO, ENAME, SAL "예전월급", SAL*1.1 "현재월급" FROM EMP;
    ...
    -- ex3. 모든 사원의 ENANE, MGR(상사번호)을 출력-상사사번이 없으면 'CEO'
...
-- (1) BETWEEN A AND B :A부터 B까지(A, B포함 A<=B)
    ...
    -- ex1. SAL이 1500미만 3000초과 (ex1의 반대)
