...
// now.getNumberOfDays(openday) => now는 this/ openday는 thats
Date.prototype.getNumberOfDays = function(that){
    let intevalMilisec = Math.abs( this.getTime() - that.getTime());//절대값
    let day = Math.floor (intevalMilisec / (1000*60*60*24));//내림
    return day;
};
// let now =new Date();
